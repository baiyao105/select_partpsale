...
export default function App() {
  ...
  const {
    ...
    isError,
    error,
  } = useDeviceQuery(queryCode);
  const { data: insuranceData } = useInsuranceQuery(queryCode);
  ...
  const setStatus = useCallback(
    (text: string, type: "info" | "ok" | "err" = "info") => {
  ...
  useEffect(() => {
    if (isLoading) setStatus("查询中...", "info");
  }, [isLoading, setStatus]);
  useEffect(() => {
    if (adbError) setStatus(adbError, "err");
  }, [adbError, setStatus]);
  useEffect(() => {
    if (isError)
      setStatus(error instanceof Error ? error.message : "查询失败", "err");
  }, [isError, error, setStatus]);
  useEffect(() => {
    if (queryData && !isLoading && !isError) {
      const ok = !!(queryData.data && Object.keys(queryData.data).length > 0);
      setStatus(
        ok ? "查询成功" : queryData.msg || "未找到数据",
        ok ? "ok" : "err",
      );
    }
  ...
  const handleScanResult = useCallback(
    (text: string) => {
      const bn = extractBindNumber(text);
      setInput(bn);
      doQuery(bn);
    },
    [doQuery],
  ...
  const handleScanError = useCallback(
    (msg: string) => setStatus("摄像头错误: " + msg, "err"),
    [setStatus],
  );
  ...
  return (
    <div className={`app ${hasResult || isLoading ? "app-expanded" : ""}`}>
