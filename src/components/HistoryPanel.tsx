...
export function HistoryPanel({ history, onSelect, onRemove, onClearAll }: Props) {
  ...
  return (
    <div className={`expander history-panel ${open ? 'open' : ''}`}>
      <div className="expander-head" onClick={() => setOpen(!open)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') setOpen(!open); }}>
        <div className="expander-icon gray"><Clock size={16} /></div>
          ...
          >
            <div className="expander-content">
              <div className="history-actions">
                <button className="history-clear-all" onClick={onClearAll}><Trash2 size={12} />清空全部</button>
              </div>
              {history.map((code) => (
                <div key={code} className="history-item" onClick={() => onSelect(code)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') onSelect(code); }}>
                  <span className="history-code">{code}</span>
                  <button className="history-remove" onClick={(e) => { e.stopPropagation(); onRemove(code); }} title="删除">
                    <X size={12} />
                  </button>
                </div>
              ))}
